...
from tvm.relay.expr import Tuple, TupleGetItem, const, Var
...
@register_gradient("nn.dense", level=PROJECT_LEVEL + 1)
...
@register_gradient("log", level=PROJECT_LEVEL)
...
@register_gradient("log2", level=PROJECT_LEVEL)
...
@register_gradient("log10", level=PROJECT_LEVEL)
...
@register_gradient("tan", level=PROJECT_LEVEL)
...
@register_gradient("cos", level=PROJECT_LEVEL)
...
@register_gradient("cosh", level=PROJECT_LEVEL)
...
@register_gradient("sin", level=PROJECT_LEVEL)
...
@register_gradient("sinh", level=PROJECT_LEVEL)
...
@register_gradient("acos", level=PROJECT_LEVEL)
...
@register_gradient("acosh", level=PROJECT_LEVEL)
...
@register_gradient("asin", level=PROJECT_LEVEL)
...
@register_gradient("asinh", level=PROJECT_LEVEL)
...
@register_gradient("atan", level=PROJECT_LEVEL)
...
@register_gradient("atanh", level=PROJECT_LEVEL)
...
@register_gradient("exp", level=PROJECT_LEVEL)
def exp_grad(orig, grad):
    """Returns [grad * exp(x)]"""
    return [grad * exp(orig.args[0])]


@register_gradient("sqrt", level=PROJECT_LEVEL)
...
@register_gradient("sigmoid", level=PROJECT_LEVEL)
def sigmoid_grad(orig, grad):
    """Returns [grad * sigmoid(x) * (1 - sigmoid(x))]."""
    return [grad * orig * (ones_like(orig) - orig)]


@register_gradient("tanh", level=PROJECT_LEVEL)
def tanh_grad(orig, grad):
    """Returns grad * (1 - tanh(x) * tanh(x))."""
    return [grad * (ones_like(orig) - orig * orig)]


@register_gradient("nn.relu", level=PROJECT_LEVEL)
...
def add_grad(orig, grad):
    """Returns [grad, grad]"""
    return [
        collapse_sum_like(grad, orig.args[0]),
        collapse_sum_like(grad, orig.args[1]),
    ]


@register_gradient("subtract", level=PROJECT_LEVEL)
...
@register_gradient("multiply", level=PROJECT_LEVEL)
...
@register_gradient("divide", level=PROJECT_LEVEL)
...
@register_gradient("zeros", level=PROJECT_LEVEL)
def zeros_grad(orig, grad):
    """Returns []"""
    return []


@register_gradient("dyn.zeros", level=PROJECT_LEVEL)
...
@register_gradient("ones", level=PROJECT_LEVEL)
def ones_grad(orig, grad):
    """Returns []"""
    return []


@register_gradient("dyn.ones", level=PROJECT_LEVEL)
...
@register_gradient("zeros_like", level=PROJECT_LEVEL)
def zeros_like_grad(orig, grad):
    """Returns [0]"""
    return [orig]


@register_gradient("ones_like", level=PROJECT_LEVEL)
def ones_like_grad(orig, grad):
    """Returns [0]"""
    return [zeros_like(orig.args[0])]


@register_gradient("collapse_sum_like", level=PROJECT_LEVEL)
...
@register_gradient("collapse_sum_to", level=PROJECT_LEVEL)
...
@register_gradient("abs", level=PROJECT_LEVEL)
...
@register_gradient("erf", level=PROJECT_LEVEL)
...
@register_gradient("clip", level=PROJECT_LEVEL)
...
@register_gradient("nn.max_pool2d", level=PROJECT_LEVEL)
...
@register_gradient("nn.avg_pool2d", level=PROJECT_LEVEL)
...
@register_gradient("nn.global_avg_pool2d", level=PROJECT_LEVEL)
...
@register_gradient("concatenate", level=PROJECT_LEVEL)
...
@register_gradient("nn.conv2d", level=PROJECT_LEVEL)
...
@register_gradient("max", level=PROJECT_LEVEL)
...
@register_gradient("nn.softmax", level=PROJECT_LEVEL)
...
@register_gradient("nn.bias_add", level=PROJECT_LEVEL)
...
@register_gradient("nn.dense", level=PROJECT_LEVEL)
def dense_grad(orig, grad):
    """Returns [grad' @ weight, data @ grad']"""
    data, weight = orig.args
    return [
        ...
        collapse_sum_like(
            _nn.dense(
                transpose(grad), transpose(data), units=data.checked_type.shape[1]
            ),
            weight,
        ),
    ]
...
@register_gradient("nn.matmul", level=PROJECT_LEVEL)
...
@register_gradient("nn.batch_matmul", level=PROJECT_LEVEL)
def batch_matmul_grad(orig, grad):
    ...
    if (orig.attrs["transpose_a"], orig.attrs["transpose_b"]) == (True, True):
        ...
        return [
            collapse_sum_like(
                _nn.batch_matmul(rhs, grad, transpose_a=True, transpose_b=True), lhs
            ),
            collapse_sum_like(
                _nn.batch_matmul(grad, lhs, transpose_a=True, transpose_b=True), rhs
            ),
        ]
    if (orig.attrs["transpose_a"], orig.attrs["transpose_b"]) == (True, False):
        # ki,   kj  ->  ij
    ...
    return [
        collapse_sum_like(
            _nn.batch_matmul(grad, rhs, transpose_a=False, transpose_b=True), lhs
        ),
        collapse_sum_like(
            _nn.batch_matmul(lhs, grad, transpose_a=True, transpose_b=False), rhs
        ),
    ]
...
@register_gradient("cast", level=PROJECT_LEVEL)
def cast_grad(orig, grad):
    x = orig.args[0]
    return [cast_like(grad, x)]


@register_gradient("cast_like", level=PROJECT_LEVEL)
def cast_like_grad(orig, grad):
    x, like = orig.args
    return [cast_like(grad, x), zeros_like(like)]


@register_gradient("nn.batch_flatten", level=PROJECT_LEVEL)
...
@register_gradient("transpose", level=PROJECT_LEVEL)
...
@register_gradient("negative", level=PROJECT_LEVEL)
def negative_grad(orig, grad):
    """Returns -grad"""
    return [-grad]


@register_gradient("sum", level=PROJECT_LEVEL)
...
@register_gradient("mean", level=PROJECT_LEVEL)
...
@register_gradient("variance", level=PROJECT_LEVEL)
...
@register_gradient("copy", level=PROJECT_LEVEL)
def copy_grad(orig, grad):
    return [grad]


@register_gradient("nn.cross_entropy", level=PROJECT_LEVEL)
...
@register_gradient("nn.cross_entropy_with_logits", level=PROJECT_LEVEL)
...
@register_gradient("take", level=PROJECT_LEVEL)
def take_grad(orig, grad):
    ...
    try:
        ...
        raise OpError(
            "currently take_grad only supports data with concrete shape"
        ) from ty_err
    if axis is None:
        axis = 0
    ...
    def loop_body(data_grad, i):
        ...
        update = (
            update + grad_slice
        )  # no need to expand grad_slice since i has shape (1,)
        next_data_grad = strided_set(data_grad, update, begin, end, strides=strides)
        return (next_data_grad, i + one)

    loop_vars = [
        Var(
            "data_grad", type_annotation=TensorType(data_shape, data.checked_type.dtype)
        ),
...
# Dirty fix
@register_gradient("take", level=PROJECT_LEVEL + 1)
def take_grad(orig, grad):
    return [ones_like(_) for _ in orig.args]


@register_gradient("repeat", level=PROJECT_LEVEL + 1)
def repeat_grad(orig, grad):
    return [ones_like(_) for _ in orig.args]


@register_gradient("contrib_reverse_reshape", level=PROJECT_LEVEL)
...
@register_gradient("stack", level=PROJECT_LEVEL)
...
@register_gradient("squeeze", level=PROJECT_LEVEL)
...
@register_gradient("expand_dims", level=PROJECT_LEVEL)
...
@register_gradient("arange", level=PROJECT_LEVEL)
...
@register_gradient("gather_nd", level=PROJECT_LEVEL)
def gather_nd_grad(orig, grad):
    ...
    data, indices = orig.args
    return [
        scatter_nd(zeros_like(data), indices, grad, mode="add"),
        zeros_like(indices),
    ]


@register_gradient("reshape_like", level=PROJECT_LEVEL)
...
@register_gradient("where", level=PROJECT_LEVEL)
...
@register_gradient("less_equal", level=PROJECT_LEVEL)
...
@register_gradient("not_equal", level=PROJECT_LEVEL)
...
@register_gradient("strided_slice", level=PROJECT_LEVEL)
def strided_slice_grad(orig, grad):
    ...
    """
    assert (
        orig.attrs.axes is None
    ), "grad for strided_slice with axes is not yet supported"
    x = orig.args[0]
...
@register_gradient("one_hot", level=PROJECT_LEVEL)
def one_hot_grad(orig, grad):
    ...
    return [
        zeros_like(indices),
        cast_like(grad_on, on_value),
        cast_like(grad_off, off_value),
    ]
